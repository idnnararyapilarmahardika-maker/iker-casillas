...
// ===== SCROLL SNAP =====
// Aktifkan scroll snap hanya di desktop
function applyScrollSnap() {
    if (window.innerWidth >= 768) {
        document.documentElement.style.scrollSnapType = 'y proximity';
        document.querySelectorAll('section, nav + div').forEach(section => {
...
// Stagger animation untuk children elemen
document.querySelectorAll('.grid, .space-y-4').forEach(container => {
    const children = container.querySelectorAll(':scope > *:not(.reveal):not(.reveal-scale):not(.reveal-left):not(.tilt-card):not(.trophy-3d)');
...
// ===== SCROLL-TRIGGERED COUNTER UNTUK HERO =====
const heroStats = document.querySelectorAll('#hero .text-2xl.font-black');
